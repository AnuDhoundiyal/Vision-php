...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `profile_image` VARCHAR(255) DEFAULT NULL, -- Stores path relative to /uploads
...
CREATE TABLE IF NOT EXISTS `attendance` (
  ...
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `unique_attendance_per_day_per_course` (`user_id`, `user_type`, `date`, `course_id`), -- Ensure unique attendance per user per day per course
  FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE CASCADE,
  FOREIGN KEY (`course_id`) REFERENCES `courses`(`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
